package com.drebate.utils;


import com.drebate.exception.BizException;
import com.drebate.exception.BizExceptionCodeEnum;
...
public class ValidateUtils {
    ...
    public static <T> void validateLogic(boolean flag, BizExceptionCodeEnum bizExceptionCodeEnum) {
        if (!flag) {
            throw new BizException(bizExceptionCodeEnum.getCode(),bizExceptionCodeEnum.getMessage());
        }
    }

    public static <T> void validateLogic(boolean flag, BizExceptionCodeEnum bizExceptionCodeEnum, String message) {
        if (!flag) {
            throw new BizException(bizExceptionCodeEnum.getCode(), StringUtils.isNotBlank(message) ? message : bizExceptionCodeEnum.getMessage());
        }
    }

    public static <T> void validateLogic(T t, Predicate<T> predicate, BizExceptionCodeEnum bizExceptionCodeEnum) {
        if (!predicate.test(t)) {
            throw new BizException(bizExceptionCodeEnum.getCode(), bizExceptionCodeEnum.getMessage());
        }
    }
